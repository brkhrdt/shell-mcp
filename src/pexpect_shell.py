...
# --- Example Usage ---
def main():
    # Example 1: Linux/macOS Bash
    if os.name == 'posix':
        log.info("\n--- Running Pexpect Bash Example ---")
        bash_path = shutil.which("bash")
        if bash_path:
            bash_shell = PexpectInteractiveShell([bash_path, "--norc", "-i"], prompt_patterns=[r'bash.*\$'])
            try:
                bash_shell.start()
                
                log.info("\n--- Testing 'pwd' ---")
                pwd_output = bash_shell.run_command("pwd")
                log.info(f"PWD Output: '{pwd_output}'")

                log.info("\n--- Testing 'echo hello world' ---")
                output1 = bash_shell.run_command("echo hello world")
    ...
    # Example 2: Windows Command Prompt
    if os.name == 'nt':
        log.info("\n--- Running Pexpect CMD.exe Example ---")
        cmd_shell = PexpectInteractiveShell(["cmd.exe"])
        try:
            cmd_shell.start()

            log.info("\n--- Testing 'echo hello world' (Windows) ---")
            output_win1 = cmd_shell.run_command("echo hello world")
